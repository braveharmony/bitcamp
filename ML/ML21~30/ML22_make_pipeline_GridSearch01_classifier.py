...
def pipeline(x,y,scaler=MinMaxScaler,classifier=RandomForestClassifier):
    ...
    # 2. model build
    pipe=make_pipeline(scaler(),PCA(x_train.shape[1]//2),classifier())
    def mergedict(a,b):
        parameterlists = [{f"{classifier.__name__.lower()}__n_estimators":[100,200]}]
        parameterlists.append({f'{classifier.__name__.lower()}__max_depth':[6,8,10,12]})
        parameterlists.append({f'{classifier.__name__.lower()}__min_samples_leaf':[3,5,7,10]})
        parameterlists.append({f'{classifier.__name__.lower()}__min_samples_split':[2,3,5,10]})
        parameterlists.append({f'{classifier.__name__.lower()}__n_jobs':[-1,2,4]})
...
for i in [load_iris,load_digits,load_breast_cancer]:
    ...
    for j in [MinMaxScaler,MaxAbsScaler,StandardScaler,RobustScaler]:
        for k in [RandomForestClassifier,DecisionTreeClassifier]:
