...
def pipeline(x,y,scaler=MinMaxScaler,regressor=RandomForestRegressor):
    ...
    # 2. model build
    pipe=make_pipeline(scaler(),PCA(x_train.shape[1]//2),regressor())
    def mergedict(a,b):
        parameterlists = [{f"{regressor.__name__.lower()}__n_estimators":[100,200]}]
        parameterlists.append({f'{regressor.__name__.lower()}__max_depth':[6,8,10,12]})
        parameterlists.append({f'{regressor.__name__.lower()}__min_samples_leaf':[3,5,7,10]})
        parameterlists.append({f'{regressor.__name__.lower()}__min_samples_split':[2,3,5,10]})
        parameterlists.append({f'{regressor.__name__.lower()}__n_jobs':[-1,2,4]})
...
for i in [fetch_california_housing,load_diabetes]:
    ...
    for j in [MinMaxScaler,MaxAbsScaler,StandardScaler,RobustScaler] :
        for k in [RandomForestRegressor,DecisionTreeRegressor]:
...
# ===============================================================
# fetch_california_housing model : RandomForestRegressor scaler : MinMaxScaler score : 0.7378295499321963
# {'randomforestregressor__min_samples_leaf': 3, 'randomforestregressor__n_estimators': 200}
# ===============================================================
# load_diabetes model : RandomForestRegressor scaler : StandardScaler score : 0.3300739740366896
# {'randomforestregressor__max_depth': 6, 'randomforestregressor__min_samples_leaf': 5}