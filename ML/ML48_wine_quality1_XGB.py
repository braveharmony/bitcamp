...
model.add(Dense(512,activation=LeakyReLU(0.5)))
...
model.fit(x_train,y_train,validation_data=(x_test,y_test),epochs=10000,batch_size=len(x_train)//99,
          callbacks=es(monitor='val_loss',mode='min',patience=50,restore_best_weights=True,verbose=True))

y_pred=np.argmax(model.predict(dft),axis=1)+3
