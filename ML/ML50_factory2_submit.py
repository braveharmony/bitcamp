...
le=LabelEncoder()
train_dataset['location']=le.fit_transform(train_dataset['측정소'])
...
for i in range(3):
    x_train,x_test,y_train,y_test=train_test_split(x,y,train_size=0.8,random_state=i,shuffle=True)
    x_predict=test_input_dataset.drop(test_input_dataset[test_input_dataset['PM2.5'].notna()].index)
    x_predict=x_predict.drop(['PM2.5'],axis=1)
    print(x_predict)

    parameters={'n_estimators' : 200,#디폴트 100,1~inf,int
    ...
    model = XGBRegressor(**parameters, eval_metric='mae')
    import time
    ...
    sub=pd.read_csv(file_path()+'answer_sample.csv')
    sub['PM2.5']=y_predict
    sub.to_csv(f'./ML/sample_submit{i}.csv',index=False)