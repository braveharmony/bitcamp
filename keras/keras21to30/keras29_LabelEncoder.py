...
# df.profile_report().to_file('./pr_report.html')
...
layer = Dense(32,activation='relu')(input1)
...
# model.summary()
model.save(f'./_save/dacon_wine/03_15/model{tc}.h5')
# 3. 컴파일,훈련
from tensorflow.python.keras.callbacks import EarlyStopping
model.compile(loss='categorical_crossentropy',optimizer='adam',metrics='acc')
hist=model.fit(x_train,y_train,batch_size=len(x_train)//100,epochs=10000
          ,validation_split=0.2,verbose=True
          ,callbacks=EarlyStopping(monitor='val_loss',mode='min',patience=400,restore_best_weights=True,verbose=True))
...
dfs.to_csv(f'./_save/dacon_wine/03_15/forsub{tc}.csv',index=False)
model.save(f'./_save/dacon_wine/03_15/forsub{tc}.h5')
