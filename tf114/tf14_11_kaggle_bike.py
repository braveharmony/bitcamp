...
x=datasets.drop([datasets.columns[-1],datasets.columns[-2],datasets.columns[-3]],axis=1)
y=datasets[datasets.columns[-1]].values
print(x.shape)
y=y.reshape(-1,1)
...
w1 = tf.compat.v1.Variable(tf.compat.v1.random_normal([8, 32]), name='weight1')
b1 = tf.compat.v1.Variable(tf.compat.v1.zeros([32]), name='bias1')
layer1 = tf.compat.v1.matmul(xp, w1) + b1                                                           

layer1 = tf.compat.v1.nn.leaky_relu(layer1, alpha=0.5)

w2 = tf.compat.v1.Variable(tf.compat.v1.random_normal([32, 32]), name='weight2')
b2 = tf.compat.v1.Variable(tf.compat.v1.zeros([32]), name='bias2')
layer2 = tf.compat.v1.matmul(layer1, w2) + b2

layer2 = tf.compat.v1.nn.leaky_relu(layer2, alpha=0.5)

w3 = tf.compat.v1.Variable(tf.compat.v1.random_normal([32, 1]), name='weight3')
