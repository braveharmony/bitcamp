...
# 1. data prepare
import pandas as pd
datasets=pd.read_csv('./_data/dacon_wine/train.csv',index_col=0)
print(datasets)
print(datasets.info())
from sklearn.preprocessing import LabelEncoder
incoder=LabelEncoder()
for column in datasets.columns:
    if datasets[column].dtype=='object':
        datasets[column]=incoder.fit_transform(datasets[column])
x_data=datasets.drop('quality',axis=1)
y_data=datasets['quality']
